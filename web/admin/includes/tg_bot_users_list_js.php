...
$res .= '
	...
	<div class="adm_zag">First name</div>
	<div class="adm_zag">Last name</div>
...
$query="SELECT * FROM tg_bot_users";

if ( strlen($busc) > 0 ) {
	$query .= " WHERE 
			id			LIKE '%".$busc."%' OR 
			username	LIKE '%".$busc."%' OR 
			first_name	LIKE '%".$busc."%' OR 
			last_name	LIKE '%".$busc."%'";
}
...
while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
	...
	$res .= '
		...
		<div class="adm_list_txt" id="c1_'.$row['id'].'">'.$row['username'].'</div>
		<div class="adm_list_txt" id="c2_'.$row['id'].'">'.$row['first_name'].'</div>
		<div class="adm_list_txt" id="c3_'.$row['id'].'">'.$row['last_name'].'</div>
		<div class="adm_list_txt" id="c4_'.$row['id'].'">'.$created_at_human.'</div>
		<div class="adm_list_txt" id="c5_'.$row['id'].'">'.$updated_at_human.'</div>
