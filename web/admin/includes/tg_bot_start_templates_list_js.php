...
$res .= '<div class="adm_busc">
	...
	<div class="adm_busc_bt" onclick="template_list_by_filter();">Find</div>
...
$res .= '
	...
	<div class="adm_zag">Template</div>
...
$query="SELECT * FROM tg_bot_start_templates";

if ( strlen($busc) > 0 ) {
	$query .= " WHERE parameter LIKE '%".$busc."%'";
}
...
while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
	...
	$res .= '
		...
		<div class="adm_list_txt" id="c1_'.$row['id'].'">'.$row['parameter'].'</div>

		<div class="adm_list_txt" id="c2_'.$row['id'].'">'.$row['template'].'</div>

		<div class="adm_list_txt pad" id="c3_'.$row['id'].'">
			<img id="edit_icon_'.$row['id'].'" onclick="template_get_edit_form('.$row['id'].')" src="img/edit.png" class="edit-icon-size">
        </i></div>

		<div class="adm_list_txt pad" id="c4_'.$row['id'].'">
			<img onclick="template_del('.$row['id'].')" src="img/trash.png" class="edit-icon-size">
		</div>
