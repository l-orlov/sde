...
// Todos los productos y servicios de la empresa para slides "Exportable products" (un slide por cada uno; datos siempre desde BD)
$productosParaSlides = [];
if (!empty($companyIds)) {
    $ids = implode(',', array_map('intval', $companyIds));
...
if (!empty($companyIds)) {
    ...
    $q = 'SELECT company_id, description';
    if ($productHasCol('description_en')) {
        $q .= ', description_en';
    }
    $descWhere = "(description IS NOT NULL AND description != '')";
    if ($productHasCol('description_en')) {
        $descWhere .= " OR (description_en IS NOT NULL AND description_en != '')";
    }
    $q .= " FROM products WHERE company_id IN ($ids) AND ($descWhere) ORDER BY company_id, id ASC";
    $r = mysqli_query($link, $q);
    if ($r) {
        while ($row = mysqli_fetch_assoc($r)) {
            $cid = (int) $row['company_id'];
...
if (!empty($companyIds)) {
    ...
    $tmSel = [];
    $tmWhere = [];
    if ($productHasCol('target_markets')) {
        $tmSel[] = 'target_markets';
        $tmWhere[] = "(target_markets IS NOT NULL AND target_markets != '' AND target_markets != '[]')";
    }
    if ($productHasCol('target_markets_en')) {
        $tmSel[] = 'target_markets_en';
        $tmWhere[] = "(target_markets_en IS NOT NULL AND target_markets_en != '' AND target_markets_en != '[]')";
    }
    if ($tmSel !== [] && $tmWhere !== []) {
        $q = 'SELECT ' . implode(', ', $tmSel) . " FROM products WHERE company_id IN ($ids) AND (" . implode(' OR ', $tmWhere) . ')';
        $res = @mysqli_query($link, $q);
        if ($res) {
            while ($row = mysqli_fetch_assoc($res)) {
                $raw = !empty(trim((string)($row['target_markets_en'] ?? ''))) ? $row['target_markets_en'] : ($row['target_markets'] ?? null);
...
for ($i = 0; $i < count($htmlChunks); $i++) {
    if ($i === 0) {
        ...
        $s1ActivityY = $s1Ty + 48;
        $s1MainActivity = !empty(trim((string)($companies[0]['main_activity_en'] ?? ''))) ? trim((string) $companies[0]['main_activity_en']) : (!empty($companies[0]['main_activity']) ? trim((string) $companies[0]['main_activity']) : '');
        if ($s1MainActivity === '') {
            foreach ($rubros as $r) {
                if ($r !== '' && $r !== 'Otros sectores') {
                    $s1MainActivity = $r;
                    break;
                }
            }
        }
        if ($s1MainActivity === '' && !empty($productosMuestra[0]['activity'])) {
            $s1MainActivity = trim((string) $productosMuestra[0]['activity']);
        }
        if ($s1MainActivity !== '') {
            $s1ProvText = function_exists('mb_strtoupper') ? mb_strtoupper($s1MainActivity, 'UTF-8') : strtoupper($s1MainActivity);
        } else {
            $s1ProvText = function_exists('mb_strtoupper') ? mb_strtoupper('MAIN ACTIVITY', 'UTF-8') : 'MAIN ACTIVITY';
        }
        $mpdf->SetFont($pdfFontFamily, 'B', 16);
        $mpdf->SetXY($s1TextLeft, $s1ActivityY);
        $mpdf->SetTextColor(255, 255, 255);
        $mpdf->MultiCell($s1TextW, 5, $s1ProvText, 0, 'L');
        ...
        $mpdf->Cell($pfLeftW - 2 * $pfPad, 16, 'COMPANY', 0, 1, 'L');
        $mpdf->SetTextColor(0, 0, 0);
        $mpdf->SetFont($pdfFontFamily, 'B', 52);
        $mpdf->SetXY($pfPad, $pfTitleY + 20);
        $mpdf->Cell($pfLeftW - 2 * $pfPad, 18, 'PROFILE', 0, 1, 'L');
        $pfPerfil1 = 'Organization type';
        $pfPerfil2 = !empty(trim($pfEmp['organization_type_en'] ?? '')) ? trim($pfEmp['organization_type_en']) : (trim($pfEmp['organization_type'] ?? '') ?: '-');
        ...
        $pfChannelsLines = array_merge([$pfWebStr], $pfRedesLines);
        ...
        $pfRowHeights = [44, 56, 44];
        $pfColW = ($pfLeftW - 2 * $pfPad - 12) / 2;
        $pfGap = 12;
        $pfRows = [
            ['01', 'PROFILE:', [$pfPerfil1, $pfPerfil2]],
            ['02', 'LOCATION:', [$pfLoc, $pfDepto, $pfDomicilio]],
            ['03', 'CHANNELS:', $pfChannelsLines],
            ['04', 'CONTACT:', [$pfCargo, $pfEmail, $pfTel]],
        ];
        $pfNumToTextGap = 6;
        foreach ($pfRows as $idx => $row) {
            $col = $idx % 2;
            $rowIdx = (int)floor($idx / 2);
            $by = $pfBlockY + array_sum(array_slice($pfRowHeights, 0, $rowIdx));
        ...
        $s4TitleLine1 = 'EXPORTABLE';
        $s4TitleLine2 = 'PRODUCTS AND SERVICES';
        ...
        foreach ($productosParaSlides as $p6PageIdx => $prod) {
            ...
            $p6ProductName = trim((string) ($prod['name_en'] ?? '')) !== '' ? trim((string) $prod['name_en']) : trim((string) ($prod['name'] ?? ''));
            $p6TitleDisplay = $p6ProductName !== '' ? (function_exists('mb_strtoupper') ? mb_strtoupper($p6ProductName, 'UTF-8') : strtoupper($p6ProductName)) : '';
            $p6TitleText = $p6TitleDisplay !== '' ? $p6TitleDisplay : 'PRODUCT/SERVICE';
            $p6TitleMaxW = max(50.0, $p6LogoX - $p6Pad - 8);
            $mpdf->SetTextColor($p6RedR, $p6RedG, $p6RedB);
            $p6TitleFont = 52;
            $mpdf->SetFont($pdfFontFamily, 'B', $p6TitleFont);
            $p6Uw = $mpdf->GetStringWidth($p6TitleText);
            if ($p6Uw > $p6TitleMaxW * 1.05) {
                $p6TitleFont = 40;
                $mpdf->SetFont($pdfFontFamily, 'B', $p6TitleFont);
                $p6Uw = $mpdf->GetStringWidth($p6TitleText);
            }
            if ($p6Uw > $p6TitleMaxW * 2.2) {
                $p6TitleFont = 30;
                $mpdf->SetFont($pdfFontFamily, 'B', $p6TitleFont);
                $p6Uw = $mpdf->GetStringWidth($p6TitleText);
            }
            if ($p6Uw > $p6TitleMaxW * 3.5) {
                $p6TitleFont = 24;
                $mpdf->SetFont($pdfFontFamily, 'B', $p6TitleFont);
            }
            $p6TitleLineH = max(8, round($p6TitleFont * 0.35));
            $mpdf->SetXY($p6Pad, $p6TitleY);
            $mpdf->MultiCell($p6TitleMaxW, $p6TitleLineH, $p6TitleText, 0, 'L');
            $p6ImgGap = 2;
            $p6ContentW = $wMm - 2 * $p6Pad - $p6ImgGap;
            $p6ImgLeftW = round($p6ContentW * 0.48);
            $p6ImgRightW = $p6ContentW - $p6ImgLeftW;
            $p6ImgGapToCols = 10;
            $p6AfterTitleY = (float) $mpdf->y + 4;
            $p6ImgLeftY = $p6AfterTitleY + 6;
            $p6ImgRightY = $p6AfterTitleY + 14;
            ...
            $p6Annual = !empty(trim((string)($prod['annual_export_en'] ?? ''))) ? trim($prod['annual_export_en']) : (trim($prod['annual_export'] ?? '') ?: '-');
            $p6Cert = !empty(trim((string)($prod['certifications_en'] ?? ''))) ? trim($prod['certifications_en']) : (trim($prod['certifications'] ?? '') ?: '-');
            $p6Tariff = trim($prod['tariff_code'] ?? '') ?: '-';
            $p6CurrentStr = pdf_en_markets_display_string($prod['current_markets'] ?? null, $prod['current_markets_en'] ?? null);
            $p6TargetStr = pdf_en_markets_display_string($prod['target_markets'] ?? null, $prod['target_markets_en'] ?? null);
            if (mb_strlen($p6TargetStr) > 80) $p6TargetStr = mb_substr($p6TargetStr, 0, 79) . '…';
            $p6Mercados = 'Current markets: ' . $p6CurrentStr . "\n" . 'Markets of interest: ' . $p6TargetStr;
        ...
        $s2HistCid = (int) ($companies[0]['id'] ?? 0);
        $s2CorpComp = ($s2HistCid && isset($competitivenessPorEmpresa[$s2HistCid]) && is_array($competitivenessPorEmpresa[$s2HistCid]))
            ? $competitivenessPorEmpresa[$s2HistCid] : [];
        $s2Para = '';
        if (!empty($companies[0]['nuestra_historia'])) {
            $s2Para = $corpPdfCleanText($companies[0]['nuestra_historia']);
        }
        if ($s2Para === '' && !empty($s2CorpComp['company_history'])) {
            $s2Para = $corpPdfCleanText($s2CorpComp['company_history']);
        }
        if ($s2Para === '') {
            $s2Para = '—';
        }
        ...
        $mpdf->Cell($c8LeftW - 2 * $c8Pad, 18, 'COMPETITIVENESS AND', 0, 1, 'L');
        $mpdf->SetFont($pdfFontFamily, 'B', 52);
        $mpdf->SetXY($c8Pad, $c8TitleY + 20);
        $mpdf->Cell($c8LeftW - 2 * $c8Pad, 18, 'DIFFERENTIATION', 0, 1, 'L');
        $c8ItemY = $c8TitleY + 58;
        $c8IconSize = 20;
        $c8ColW = ($c8LeftW - 2 * $c8Pad - 14) / 2;
        $c8ColGap = 14;
        $c8ItemH = 38;
        $c8CompCid = (int) ($companies[0]['id'] ?? 0);
        $c8Comp = ($c8CompCid && isset($competitivenessPorEmpresa[$c8CompCid]) && is_array($competitivenessPorEmpresa[$c8CompCid]))
            ? $competitivenessPorEmpresa[$c8CompCid] : [];
        $c8Fld = static function (array $comp, string $detailKey, string $fallbackKey = '') use ($corpPdfCleanText): string {
            $d = $corpPdfCleanText($comp[$detailKey] ?? '');
            if ($d === '' && $fallbackKey !== '') {
                $d = $corpPdfCleanText($comp[$fallbackKey] ?? '');
            }
            return $d !== '' ? $d : '—';
        };
        $c8Items = [
            ['AWARDS', $c8Fld($c8Comp, 'awards_detail', 'awards')],
            ['BUSINESS ROUNDS', $c8Fld($c8Comp, 'rounds_detail', 'rounds')],
            ['COMMERCIAL REFERENCES', $c8Fld($c8Comp, 'commercial_references', '')],
            ['TRADE FAIRS', $c8Fld($c8Comp, 'fairs_detail', 'fairs')],
            ['EXPORT EXPERIENCE', $c8Fld($c8Comp, 'export_experience', '')],
        ];
        ...
        $mpdf->Cell($s7LeftW - 2 * $s7Pad, 20, 'THANK YOU', 0, 1, 'L');
        $contacto = $configInstitucional;
        $s7ContactoPersona = trim($contacto['area_responsable'] ?? '') ?: 'Contact person';
