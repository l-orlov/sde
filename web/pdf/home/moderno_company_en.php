...
header('Expires: 0');
$stmt = mysqli_prepare($link, "SELECT c.id, c.name, c.name_en, c.main_activity, c.main_activity_en, c.website, c.start_date, c.organization_type, c.organization_type_en FROM companies c WHERE c.user_id = ? AND c.moderation_status = 'approved' LIMIT 1");
if ($stmt) {
...
for ($i = 0; $i < count($htmlChunks); $i++) {
    if ($i === 0) {
        ...
        foreach ($productosParaSlides as $p6PageIdx => $prod) {
            ...
            $mpdf->Cell(0, 18, $p6TitleDisplay !== '' ? $p6TitleDisplay : 'PRODUCT/SERVICE', 0, 1, 'L');
            $p6ImgGap = 2;
        ...
        $mpdf->Cell($s2TitleW, 18, 'OUR', 0, 1, 'C');
        $mpdf->SetFont($pdfFontFamily, 'B', 52);
        $mpdf->SetXY($s2TitleLeft, $s2TitleTop + 20);
        $mpdf->Cell($s2TitleW, 18, 'HISTORY', 0, 1, 'C');
