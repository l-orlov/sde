...
for ($i = 0; $i < count($htmlChunks); $i++) {
    if ($i === 0) {
        ...
        $mpdf->Cell($pfLeftW - 2 * $pfPad, 12, 'COMPANY', 0, 1, 'L');
        ...
        foreach ($productosParaSlides as $p6PageIdx => $prod) {
            ...
            $p6TipoLabel = $p6IsService ? 'SERVICE' : 'PRODUCT';
            $p6TitleLeft = 'NAME OF ' . $p6TipoLabel . ':';
            ...
            $p6ColTitles = ['01. ANNUAL EXPORT (USD):', '02. CERTIFICATIONS', '03. TARIFF CODE (NCM/HS):', '04. MARKETSOF INTEREST'];
        ...
        $mpdf->Cell($s2TitleW, 12, 'OUR', 0, 1, 'C');
        $mpdf->SetFont('dejavusans', 'B', 32);
        $mpdf->SetXY($s2TitleLeft, $s2TitleTop + 14);
        $mpdf->Cell($s2TitleW, 12, 'HISTORY', 0, 1, 'C');
        ...
        $mpdf->Cell($c8LeftW - 2 * $c8Pad, 13, 'COMPETITIVENESS AND', 0, 1, 'L');
        $mpdf->SetFont('dejavusans', 'B', 40);
        $mpdf->SetXY($c8Pad, $c8TitleY + 16);
        $mpdf->Cell($c8LeftW - 2 * $c8Pad, 14, 'DIFFERENTIATION', 0, 1, 'L');
        $c8ItemY = $c8TitleY + 58;
        $c8IconSize = 20;
