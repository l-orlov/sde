...
// Competitividad (slide 8): datos desde company_data.competitiveness (JSON)
...
for ($i = 0; $i < 5; $i++) {
    if ($i === 0) {
        ...
        $companyName = !empty($companies[0]['name']) ? trim((string) $companies[0]['name']) : 'EMPRESA';
        $companyTitle = function_exists('mb_strtoupper') ? mb_strtoupper($companyName, 'UTF-8') : strtoupper($companyName);
        ...
        $mpdf->SetTextColor(255, 255, 255);
        $mpdf->SetFont($pdfFontFamily, 'B', 14);
        ...
        // Para alinear el título con el margen derecho del logo grande (columna derecha)
        $pfLogoSize = 125;
        ...
        $mpdf->SetFont($pdfFontFamily, 'B', 54);
        $mpdf->Cell($pfTitleW, 18, 'PERFIL DE LA', 0, 1, 'R');
        $mpdf->SetXY($pfTitleLeft, 36);
        $mpdf->SetTextColor(0, 0, 0);
        $mpdf->SetFont($pdfFontFamily, 'B', 54);
        $mpdf->Cell($pfTitleW, 18, 'EMPRESA', 0, 1, 'R');
        $pfSecLeft = 24;
        ...
        $pfValues = [
            ...
            'CANALES' => [
                ...
                isset($redesPorEmpresa[$perfilCid]) ? implode("\n", $redesPorEmpresa[$perfilCid]) : '-',
            ],
        ...
        foreach ($productoSlidesChunks as $idx => $chunk) {
            ...
            $prod = $chunk[0];
            $pid = (int) $prod['id'];
            // Formatear mercados actuales e interés para mostrar en el PDF
            ...
            $prodTitleName = trim((string)($prod['name'] ?? '')) !== '' ? (string)($prod['name'] ?? '') : 'PRODUCTO/SERVICIO';
            ...
            if (file_exists($iconYesPath)) {
                ...
                $mpdf->Cell($prodDataGap, $prodRowLineH, '', 0, 0, 'L');
                $mpdf->SetFont($pdfFontFamily, '', 15);
                $mpdf->Cell(0, $prodRowLineH, $prodCurrentMarketsStr, 0, 1, 'L');
                $prodRowY += $prodRowH;
                $mpdf->Image($iconYesPath, $prodContentX, $prodRowY + $prodIconOffsetY, $prodIconW, $prodIconH);
                $mpdf->SetXY($prodContentX + $prodIconW + $prodLabelGap, $prodRowY + $prodTextOffsetY);
                $mpdf->SetFont($pdfFontFamily, 'B', 17);
                ...
                $mpdf->Cell($prodDataGap, $prodRowLineH, '', 0, 0, 'L');
                $mpdf->SetFont($pdfFontFamily, '', 15);
                $mpdf->Cell(0, $prodRowLineH, $prodTargetMarketsStr, 0, 1, 'L');
            } else {
                $prodRowY = $prodRowY0;
                $mpdf->SetXY($prodContentX + $prodIconW + $prodLabelGap, $prodRowY + $prodTextOffsetY);
                $mpdf->SetFont($pdfFontFamily, 'B', 17);
        ...
        $mpdf->SetFont($pdfFontFamily, 'B', 52);
        $mpdf->Cell($s2TextW, 20, 'NUESTRA', 0, 1, 'L');
        $mpdf->SetTextColor(0, 0, 0);
        $mpdf->SetFont($pdfFontFamily, 'B', 48);
        $mpdf->Cell($s2TextW, 20, 'HISTORIA', 0, 1, 'L');
        $mpdf->Ln(14);
        $mpdf->SetTextColor(0, 0, 0);
        $s2FontSize = 16;
        $s2LineHeight = 7;
        ...
        $mpdf->SetTextColor(255, 255, 255);
        $mpdf->SetFont($pdfFontFamily, 'B', 52);
        $mpdf->Cell($wMm - 120, 20, 'COMPETITIVIDAD Y', 0, 1, 'L');
        $mpdf->SetXY($s6TitleLeft, $s6TitleY + 22);
        $mpdf->SetFont($pdfFontFamily, 'B', 52);
        $mpdf->Cell($wMm - 120, 20, 'DIFERENCIACIÓN', 0, 1, 'L');
        $s6LineY = $s6TitleY + 22 + 20 + 6;
        $s6LineMarginL = 28;
        $s6LineMarginR = 28;
        ...
        $s6FirstCompanyId = !empty($companies[0]['id']) ? (int) $companies[0]['id'] : 0;
        $s6Comp = ($s6FirstCompanyId && isset($competitivenessPorEmpresa[$s6FirstCompanyId]) && is_array($competitivenessPorEmpresa[$s6FirstCompanyId]))
            ? $competitivenessPorEmpresa[$s6FirstCompanyId]
            : [];
        $cleanText = static function ($v): string {
            $s = trim((string) $v);
            $s = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/u', '', $s);
            return $s;
        };
        $s6ColDescs = [
            $cleanText($s6Comp['awards_detail'] ?? ($s6Comp['awards'] ?? '')),
            $cleanText($s6Comp['fairs_detail'] ?? ($s6Comp['fairs'] ?? '')),
            $cleanText($s6Comp['rounds_detail'] ?? ($s6Comp['rounds'] ?? '')),
            $cleanText($s6Comp['export_experience'] ?? ''),
            $cleanText($s6Comp['commercial_references'] ?? ''),
        ];
        $s6ColCount = 5;
        $s6ColPad = 16;
        ...
        for ($col = 0; $col < $s6ColCount; $col++) {
            ...
            $mpdf->SetTextColor(0, 0, 0);
            $mpdf->SetFont($pdfFontFamily, 'B', $s6TitleFontSize);
        ...
        $mpdf->SetTextColor(255, 255, 255);
        $mpdf->SetFont($pdfFontFamily, 'B', 95);
