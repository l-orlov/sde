<?php
/**
 * PDF presentación de empresa — diseño D3 (company_pdf_d3.php).
 * Tercera variante de diseño; misma estructura de datos que D1/D2.
...
// —— Slide 1 D3: como en diseño — izq. imagen en arco + barras azules; der. EMPRESA, SECTOR/RUBRO, pills provincia y año; miniatura arriba derecha ——
$d3_landscapePath = $assetsDir . '/img_slide2.png';
if (!file_exists($d3_landscapePath)) {
    $d3_landscapePath = dirname(__DIR__) . '/oferta/assets/img_slide2.png';
}
if (!file_exists($d3_landscapePath)) {
    $d3_landscapePath = $portadaBgPath;
}
$d3_archPath = null;
if (extension_loaded('gd') && $d3_landscapePath !== null && file_exists($d3_landscapePath)) {
    $info = @getimagesize($d3_landscapePath);
    if ($info && in_array($info[2], [IMAGETYPE_JPEG, IMAGETYPE_PNG], true)) {
        $src = $info[2] === IMAGETYPE_JPEG ? @imagecreatefromjpeg($d3_landscapePath) : @imagecreatefrompng($d3_landscapePath);
        if ($src) {
            $sw = imagesx($src);
            $sh = imagesy($src);
            $d3_archPxBig = 2080;
            $d3_archPx = 1040;
            $d3_archR = (int) ($d3_archPxBig / 2);
            $d3_archEdgeWidth = 3.5;
            $dstBig = imagecreatetruecolor($d3_archPxBig, $d3_archPxBig);
            if ($dstBig) {
                imagealphablending($dstBig, false);
                imagesavealpha($dstBig, true);
                $trans = imagecolorallocatealpha($dstBig, 0, 0, 0, 127);
                imagefill($dstBig, 0, 0, $trans);
                imagecopyresampled($dstBig, $src, 0, 0, 0, 0, $d3_archPxBig, $d3_archPxBig, $sw, $sh);
                $cx = $d3_archR;
                $archR = (float) $d3_archR;
                for ($py = 0; $py < $d3_archR; $py++) {
                    for ($px = 0; $px < $d3_archPxBig; $px++) {
                        $dx = $px - $cx;
                        $dy = $py - $d3_archR;
                        $d = sqrt($dx * $dx + $dy * $dy);
                        if ($d >= $archR + $d3_archEdgeWidth) {
                            imagesetpixel($dstBig, $px, $py, $trans);
                        } elseif ($d > $archR - $d3_archEdgeWidth) {
                            $c = @imagecolorat($dstBig, $px, $py);
                            $r = ($c >> 16) & 0xFF;
                            $g = ($c >> 8) & 0xFF;
                            $b = $c & 0xFF;
                            $alpha = (int) (127 * min(1, max(0, ($d - ($archR - $d3_archEdgeWidth)) / (2 * $d3_archEdgeWidth))));
                            $pixel = (($alpha & 0x7F) << 24) | ($r << 16) | ($g << 8) | $b;
                            imagesetpixel($dstBig, $px, $py, $pixel);
                        }
                    }
                }
                $dst = imagecreatetruecolor($d3_archPx, $d3_archPx);
                if ($dst) {
                    imagealphablending($dst, false);
                    imagesavealpha($dst, true);
                    imagecopyresampled($dst, $dstBig, 0, 0, 0, 0, $d3_archPx, $d3_archPx, $d3_archPxBig, $d3_archPxBig);
                    $d3_archPath = sys_get_temp_dir() . '/d3_arch_' . uniqid() . '.png';
                    imagepng($dst, $d3_archPath);
                    imagedestroy($dst);
                }
                imagedestroy($dstBig);
            }
...
$d3_archW = 148;
$d3_archH = 178;
$d3_archX = $d3_leftMargin;
$d3_archY = 33;
$d3_linesPath = $assetsDir . '/lines_background.png';
if (!file_exists($d3_linesPath)) {
    $d3_linesPath = dirname(__DIR__) . '/oferta/assets/lines_background.png';
}
if (file_exists($d3_linesPath)) {
    $d3_linesW = $d3_archX + $d3_archW + 20;
    $mpdf->Image($d3_linesPath, 0, 0, $d3_linesW, $hMm);
}
if ($d3_archPath !== null && file_exists($d3_archPath)) {
    $mpdf->Image($d3_archPath, $d3_archX, $d3_archY, $d3_archW, $d3_archH);
    @unlink($d3_archPath);
} else {
    $mpdf->SetFillColor(230, 245, 255);
    $mpdf->RoundedRect($d3_archX, $d3_archY, $d3_archW, $d3_archH, 8, 'F');
}

$d3_rightX = $d3_archX + $d3_archW + 24;
$d3_rightW = $wMm - $d3_rightX - 24;
$mpdf->SetTextColor(0, 0, 0);
$mpdf->SetFont('dejavusans', 'B', 44);
$d3_title = $empresaNombre !== '' ? mb_strtoupper($empresaNombre) : 'EMPRESA';
$mpdf->SetXY($d3_rightX, 72);
$mpdf->Cell($d3_rightW, 14, $d3_title, 0, 1, 'L');
$mpdf->SetFont('dejavusans', 'B', 18);
$d3_sector = $sectorRubro !== '' ? mb_strtoupper($sectorRubro) : 'SECTOR / RUBRO';
$mpdf->SetXY($d3_rightX, 92);
$mpdf->Cell($d3_rightW, 10, $d3_sector, 0, 1, 'L');

$d3_pillY = 118;
$d3_pillH = 14;
$d3_pillR = 7;
$d3_pillW = 72;
$d3_provinciaText = trim($provincia . ($locality !== '' ? ' / ' . $locality : ''));
$mpdf->SetDrawColor(0, 51, 153);
$mpdf->SetFillColor(255, 255, 255);
$mpdf->RoundedRect($d3_rightX, $d3_pillY, $d3_pillW, $d3_pillH, $d3_pillR, 'FD');
$mpdf->SetTextColor(0, 0, 0);
$mpdf->SetFont('dejavusans', '', 12);
$mpdf->SetXY($d3_rightX, $d3_pillY + 2);
$mpdf->Cell($d3_pillW, 10, $d3_provinciaText, 0, 0, 'C');

$d3_anoX = $d3_rightX + $d3_pillW + 12;
$mpdf->SetFillColor(196, 52, 59);
$mpdf->SetDrawColor(196, 52, 59);
$mpdf->RoundedRect($d3_anoX, $d3_pillY, $d3_pillW, $d3_pillH, $d3_pillR, 'FD');
$mpdf->SetTextColor(255, 255, 255);
$mpdf->SetXY($d3_anoX, $d3_pillY + 2);
$mpdf->Cell($d3_pillW, 10, $ano, 0, 0, 'C');

$d3_thumbW = 42;
$d3_thumbH = 28;
$d3_thumbX = $wMm - $d3_thumbW - 22;
$d3_thumbY = 20;
$d3_thumbPath = $assetsDir . '/img_small.png';
if (!file_exists($d3_thumbPath)) {
    $d3_thumbPath = dirname(__DIR__) . '/oferta/assets/img_small.png';
}
if (file_exists($d3_thumbPath)) {
    $ts = @getimagesize($d3_thumbPath);
    if (!empty($ts[0]) && !empty($ts[1])) {
        $tr = $ts[0] / $ts[1];
        $tw = $d3_thumbW;
        $th = $d3_thumbH;
        if ($tr > $tw / $th) { $th = $tw / $tr; } else { $tw = $th * $tr; }
        $mpdf->Image($d3_thumbPath, $d3_thumbX + ($d3_thumbW - $tw) / 2, $d3_thumbY + ($d3_thumbH - $th) / 2, $tw, $th);
    } else {
        $mpdf->Image($d3_thumbPath, $d3_thumbX, $d3_thumbY, $d3_thumbW, $d3_thumbH);
    }
}
$mpdf->SetTextColor(0, 0, 0);
...
// —— Slide 2 D3: Perfil de Empresa — izq. título, párrafo, botón rojo pill, lista; der. bloque con paisaje (esquinas izq. redondeadas), círculo ventana, miniatura ——
$mpdf->AddPage();
$mpdf->SetXY(0, 0);
$mpdf->SetFillColor(255, 255, 255);
$mpdf->Rect(0, 0, $wMm, $hMm, 'F');

$d3s2_colLeftW = round($wMm * 0.52);
$d3s2_padLeft = 26;
$d3s2_padTop = 38;
$d3s2_textW = $d3s2_colLeftW - $d3s2_padLeft - 10;
$d3s2_parW = $d3s2_textW - 18;

$mpdf->SetTextColor(0, 0, 0);
$mpdf->SetFont('dejavusans', 'B', 32);
$mpdf->SetXY($d3s2_padLeft, $d3s2_padTop);
$mpdf->Cell($d3s2_textW, 11, 'Perfil de Empresa', 0, 1, 'L');

$d3s2_parY = $d3s2_padTop + 38;
$mpdf->SetFont('dejavusans', '', 12);
$mpdf->SetXY($d3s2_padLeft, $d3s2_parY);
$mpdf->MultiCell($d3s2_parW, 5.5, $perfilText !== '' ? $perfilText : 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.', 0, 'L');

$d3s2_btnY = $d3s2_parY + 40;
$d3s2_btnH = 11;
$d3s2_btnR = 5.5;
$d3s2_btnText = 'Labormmet';
$mpdf->SetFont('dejavusans', 'B', 10);
$d3s2_btnW = max(36, $mpdf->GetStringWidth($d3s2_btnText) + 18);
$d3s2_btnX = $d3s2_padLeft;
$mpdf->SetFillColor(198, 61, 66);
$mpdf->SetDrawColor(198, 61, 66);
$mpdf->RoundedRect($d3s2_btnX, $d3s2_btnY, $d3s2_btnW, $d3s2_btnH, $d3s2_btnR, 'FD');
$mpdf->SetTextColor(255, 255, 255);
$mpdf->SetXY($d3s2_btnX, $d3s2_btnY + 2.5);
$mpdf->Cell($d3s2_btnW, 6, $d3s2_btnText, 0, 0, 'C');

$d3s2_bulletY = $d3s2_btnY + $d3s2_btnH + 26;
$bulletChar = "\xE2\x80\xA2";
$mpdf->SetTextColor(0, 0, 0);
$mpdf->SetFont('dejavusans', '', 11);
foreach (array_slice($ventajas, 0, 4) as $v) {
    $mpdf->SetXY($d3s2_padLeft, $d3s2_bulletY);
    $mpdf->Cell(5, 5.5, $bulletChar, 0, 0, 'C');
    $mpdf->MultiCell($d3s2_textW - 5, 5.5, $v !== '' ? $v : 'Lorem ipsum dolor sit amet.', 0, 'L');
    $d3s2_bulletY += 7;
}

// Bloque derecho: img_slide2.png más pequeña con esquinas redondeadas; círculo con fondo blanco y borde azul; img_small con esquina inf. der. en la de la imagen grande
$d3s2_marginRight = 28;
$d3s2_marginTop = 28;
$d3s2_marginBottom = 28;
$d3s2_marginLeft = 16;
$d3s2_rightX = $d3s2_colLeftW + 8 + $d3s2_marginLeft;
$d3s2_rightW = $wMm - $d3s2_rightX - $d3s2_marginRight;
$d3s2_rightY = $d3s2_marginTop;
$d3s2_rightH = $hMm - $d3s2_marginTop - $d3s2_marginBottom;
if (file_exists($portadaImg2)) {
    $mpdf->Image($portadaImg2, $d3s2_rightX, $d3s2_rightY, $d3s2_rightW, $d3s2_rightH);
}
// Imagen circular; fondo blanco (círculo) más grande con borde azul fino
$d3s2_circleRadiusMm = 36;
$d3s2_whiteCircleRadiusMm = 42;
$d3s2_circleCenterX = $d3s2_rightX;
$d3s2_circleCenterY = $hMm / 2;
$d3s2_circlePath = null;
if (extension_loaded('gd') && file_exists($portadaImg2)) {
    $info = @getimagesize($portadaImg2);
    if ($info && in_array($info[2], [IMAGETYPE_JPEG, IMAGETYPE_PNG], true)) {
        $src = $info[2] === IMAGETYPE_JPEG ? @imagecreatefromjpeg($portadaImg2) : @imagecreatefrompng($portadaImg2);
        if ($src) {
            $sw = imagesx($src);
            $sh = imagesy($src);
            $size = min($sw, $sh);
            ...
            if ($dst) {
                ...
                for ($y = 0; $y < $outSize; $y++) {
                    for ($x = 0; $x < $outSize; $x++) {
                        ...
                        $d = sqrt($dx * $dx + $dy * $dy);
                        if ($d <= $outR + $edgeWidth) {
                            $sx = (int) round($srcX + $x / $scale);
                            $sy = (int) round($srcY + $y / $scale);
                            if ($sx >= 0 && $sx < $sw && $sy >= 0 && $sy < $sh) {
                                $c = @imagecolorat($src, $sx, $sy);
                                if (!imageistruecolor($src)) {
                                    $rgb = @imagecolorsforindex($src, $c);
                                    $c = ($rgb !== false) ? (($rgb['red'] << 16) | ($rgb['green'] << 8) | $rgb['blue']) : 0;
                                }
                                $red = ($c >> 16) & 0xFF;
                                $green = ($c >> 8) & 0xFF;
                                $blue = $c & 0xFF;
                                if ($d <= $outR - $edgeWidth) {
                                    ...
                                    $alpha = (int) (127 * min(1, max(0, ($d - ($outR - $edgeWidth)) / (2 * $edgeWidth))));
                                }
                                $pixel = (($alpha & 0x7F) << 24) | ($red << 16) | ($green << 8) | $blue;
                                imagesetpixel($dst, $x, $y, $pixel);
                            }
                        }
                    }
                }
                $tmpCircle = sys_get_temp_dir() . '/d3s2_circle_' . uniqid() . '.png';
                if (imagepng($dst, $tmpCircle)) {
                    $d3s2_circlePath = $tmpCircle;
                }
                imagedestroy($dst);
            }
            imagedestroy($src);
...
$d3s2_circleD = $d3s2_circleRadiusMm * 2;
$d3s2_circleX = $d3s2_circleCenterX - $d3s2_circleRadiusMm;
$d3s2_circleY = $d3s2_circleCenterY - $d3s2_circleRadiusMm;
$mpdf->SetFillColor(255, 255, 255);
$mpdf->Circle($d3s2_circleCenterX, $d3s2_circleCenterY, $d3s2_whiteCircleRadiusMm, 'F');
$mpdf->SetDrawColor(0, 51, 102);
$mpdf->SetLineWidth(0.1);
$mpdf->Circle($d3s2_circleCenterX, $d3s2_circleCenterY, $d3s2_whiteCircleRadiusMm, 'S');
$mpdf->SetLineWidth(0.1);
$mpdf->SetDrawColor(0, 0, 0);
if ($d3s2_circlePath !== null && file_exists($d3s2_circlePath)) {
    $mpdf->Image($d3s2_circlePath, $d3s2_circleX, $d3s2_circleY, $d3s2_circleD, $d3s2_circleD);
    @unlink($d3s2_circlePath);
}
// Miniatura: esquina inferior derecha coincide con la de la imagen grande
$d3s2_smallW = 28;
$d3s2_smallH = 20;
$d3s2_smallX = $d3s2_rightX + $d3s2_rightW - $d3s2_smallW;
$d3s2_smallY = $d3s2_rightY + $d3s2_rightH - $d3s2_smallH;
if (file_exists($portadaImgSmall)) {
    $sz = @getimagesize($portadaImgSmall);
    if (!empty($sz[0]) && !empty($sz[1])) {
        $r = $sz[0] / $sz[1];
        $w = $d3s2_smallW;
        $h = $d3s2_smallH;
        if ($r > $w / $h) { $h = $w / $r; } else { $w = $h * $r; }
        $mpdf->Image($portadaImgSmall, $d3s2_smallX + ($d3s2_smallW - $w) / 2, $d3s2_smallY + ($d3s2_smallH - $h) / 2, $w, $h);
    } else {
        $mpdf->Image($portadaImgSmall, $d3s2_smallX, $d3s2_smallY, $d3s2_smallW, $d3s2_smallH);
    }
}
