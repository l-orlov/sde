...
for ($i = 0; $i < count($htmlChunks); $i++) {
    if ($i === 0) {
        ...
        $mpdf->Cell($s3bTextW - 16, 14, 'ESTRATÉGICO', 0, 0, 'L');
        ...
        $mpdf->MultiCell($s3bParaW, 7, 'Santiago del Estero ocupa una posición clave en el Norte argentino y se integra a ejes de conectividad que vinculan la producción regional con corredores logísticos hacia el Atlántico y el Pacífico.', 0, 'L');
        $mpdf->SetXY($s3bTextPad, $mpdf->y + 6);
        $mpdf->MultiCell($s3bParaW, 7, 'Con infraestructura vial en expansión y articulación territorial, la provincia facilita el movimiento de bienes, la llegada a mercados y la generación de nuevas oportunidades de inversión.', 0, 'L');
        $s3bImgY = $hMm - $s3bImgH;
        ...
        $mpdf->Cell(32, 8, 'Página 05', 0, 0, 'R');
        $s3cLineH = 0.5;
        $s3cLineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        ...
        $mpdf->MultiCell($s3cTitleW, 13, "ESTRUCTURA PRODUCTIVA Y SECTORES\nCLAVE", 0, 'L');

        $mpdf->SetFont('dejavusans', '', 14);
        $mpdf->SetXY($s3cTextX, $mpdf->y + 8);
        $mpdf->MultiCell($s3cBodyW, 6, 'Santiago del Estero presenta una matriz productiva diversificada basada en recursos naturales, desarrollo agroindustrial, crecimiento energético y consolidación de economías regionales con proyección nacional e internacional.', 0, 'L');
        ...
        $mpdf->Cell(32, 8, 'Página 06', 0, 0, 'R');
        $s3eLineH = 0.5;
        $s3eLineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        ...
        $mpdf->Cell($s3eColW - $s3eTitleXOff, $s3eRowH, 'AGROINDUSTRIA', 0, 1, 'L');
        ...
        $mpdf->SetTextColor(255, 255, 255);
        $bulletsLeft1 = ['Algodón', 'Maíz y soja', 'Alfalfa y forrajes', 'Producción hortícola', 'Desarrollo agroindustrial'];
        ...
        $mpdf->SetFont('dejavusans', 'B', $s3eTitleFont);
        $mpdf->SetXY($s3eLeftX + $s3eTitleXOff, $s3eY);
        $mpdf->Cell($s3eColW - $s3eTitleXOff, $s3eRowH, 'GANADERÍA', 0, 1, 'L');
        $mpdf->SetDrawColor($s3eBlue[0], $s3eBlue[1], $s3eBlue[2]);
        $mpdf->Line($s3eLeftX, $s3eY + $s3eRowH + $s3eLineTopPad, $s3eLeftX + $s3eColW - $s3eLineRightInset, $s3eY + $s3eRowH + $s3eLineTopPad);
        $s3eY += $s3eRowH + $s3eLineTopPad + $s3eLineBottomPad;
        $mpdf->SetFont('dejavusans', '', $s3eBulletFont);
        $bulletsLeft2 = ['Ganadería bovina', 'Producción caprina', 'Producción porcina y aviar', 'Industria frigorífica', 'Desarrollo genético y sanidad animal.'];
        ...
        $mpdf->Cell($s3eRightColW - $s3eTitleXOff, $s3eRowH, 'REGIONALES', 0, 1, 'L');
        $mpdf->SetDrawColor($s3eBlue[0], $s3eBlue[1], $s3eBlue[2]);
        $mpdf->Line($s3eRightX, $s3eY + $s3eRowH + $s3eLineTopPad, $s3eRightX + $s3eRightColW - $s3eLineRightInset, $s3eY + $s3eRowH + $s3eLineTopPad);
        $s3eY += $s3eRowH + $s3eLineTopPad + $s3eLineBottomPad;
        $mpdf->SetFont('dejavusans', '', $s3eBulletFont);
        $bulletsRight1 = ['Producción apícola', 'Alcaparras', 'Producción forestal', 'Agricultura familiar', 'Valor agregado regional'];
        ...
        $mpdf->SetFont('dejavusans', 'B', $s3eTitleFont);
        $mpdf->SetXY($s3eRightX + $s3eTitleXOff, $s3eY);
        $mpdf->Cell($s3eRightColW - $s3eTitleXOff, $s3eRowH, 'DESARROLLO', 0, 1, 'L');
        $mpdf->SetDrawColor($s3eBlue[0], $s3eBlue[1], $s3eBlue[2]);
        $mpdf->Line($s3eRightX, $s3eY + $s3eRowH + $s3eLineTopPad, $s3eRightX + $s3eRightColW - $s3eLineRightInset, $s3eY + $s3eRowH + $s3eLineTopPad);
        $s3eY += $s3eRowH + $s3eLineTopPad + $s3eLineBottomPad;
        $mpdf->SetFont('dejavusans', '', $s3eBulletFont);
        $bulletsRight2 = ['Energías renovables', 'Infraestructura productiva', 'Sistemas de riego', 'Desarrollo territorial sostenible', 'Expansión industrial'];
        ...
        $mpdf->Cell(32, 8, 'Página 07', 0, 0, 'R');
        $s3dLineH = 0.5;
        $s3dLineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        ...
        $mpdf->MultiCell($s3dTitleW, 13, "INNOVACIÓN Y FUTURO\nPRODUCTIVO", 0, 'L');

        $mpdf->SetFont('dejavusans', '', 14);
        $mpdf->SetXY($s3dTextX, $mpdf->y + 8);
        $mpdf->MultiCell($s3dBodyW, 6, 'La provincia impulsa nuevas oportunidades basadas en innovación, economía del conocimiento, educación y transformación digital, fortaleciendo la competitividad productiva y la inserción internacional.', 0, 'L');
        ...
        $mpdf->Cell(32, 8, 'Página 08', 0, 0, 'R');
        $s3fLineH = 0.5;
        $s3fLineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        ...
        $lx = $s3fPad;
        $y = $s3fStartY;
        $drawEduItem($lx, $y, '01', "EDUCACIÓN");
        $y += $s3fItemGap;
        $drawEduItem($lx, $y, '02', "TECNOLOGÍA");
        $y += $s3fItemGap;
        $drawEduItem($lx, $y, '03', "ECONOMÍA DEL\nCONOCIMIENTO");

        // Right column (04-06)
        $rx = $s3fRightX;
        $y = $s3fStartY;
        $drawEduItem($rx, $y, '04', "FORMACIÓN\nPROFESIONAL", $s3fRightColW);
        $y += $s3fItemGap;
        $drawEduItem($rx, $y, '05', "TRANSFORMACIÓN\nDIGITAL", $s3fRightColW);
        $y += $s3fItemGap;
        $drawEduItem($rx, $y, '06', "DESARROLLO", $s3fRightColW);
        ...
        $mpdf->Cell(32, 8, 'Página 09', 0, 0, 'R');
        $s3gLineH = 0.5;
        $s3gLineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        ...
        $mpdf->Cell($s3gTextW, 13, 'TURISMO COMO MOTOR', 0, 1, 'L');
        $mpdf->SetTextColor(255, 255, 255);
        $mpdf->SetFont('dejavusans', 'B', 34);
        $mpdf->SetXY($s3gTextX, $mpdf->y + 2);
        $mpdf->Cell($s3gTextW, 13, 'ECONÓMICO', 0, 1, 'L');
        $mpdf->SetFont('dejavusans', '', 14);
        $mpdf->SetXY($s3gTextX, $mpdf->y + 10);
        $mpdf->MultiCell($s3gParaW, 6.5, 'El turismo se consolida como uno de los motores estratégicos de diversificación económica provincial, integrando naturaleza, cultura, deporte y bienestar.', 0, 'L');
        $mpdf->SetFont('dejavusans', '', 18);
        $mpdf->SetXY($s3gTextX, $mpdf->y + 45);
        $mpdf->MultiCell($s3gTextW, 8, "TURISMO Y DESARROLLO\nTERRITORIAL", 0, 'L');
        ...
        $mpdf->Cell(32, 8, 'Página 10', 0, 0, 'R');
        $s3jLineH = 0.5;
        $s3jLineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        ...
        foreach ([
            ...
            ['w' => $s3jWideW, 'img' => $termas2Path, 'srcX' => null, 'srcY' => null, 'caption' => 'TERMAS DE RÍO HONDO'],
            ...
            ['w' => $s3jWideW, 'img' => $estadioPath, 'srcX' => 'right', 'srcY' => null, 'caption' => 'ESTADIO ÚNICO MADRE DE CIUDADES'],
        ] as $idx => $panel) {
            $pw = $panel['w'];
            $px = $s3jX;
            $s3jX += $pw + $s3jGap;
        ...
        $mpdf->SetFont('dejavusans', '', 17);
        $mpdf->SetXY($wMm - $s3jPad - 36, ($s3jHeaderH - 8) / 2);
        $mpdf->Cell(32, 8, 'Página 11', 0, 0, 'R');
        $s3jLineH = 0.5;
        $s3jLineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        $mpdf->Rect($s3jPad, $s3jHeaderH - $s3jLineGap - $s3jLineH, $wMm - 2 * $s3jPad, $s3jLineH, 'F');
        ...
        foreach ([
            ...
            ['w' => $s3jWideW, 'img' => $ciudadPath, 'srcX' => 'right', 'srcY' => null, 'caption' => 'CIUDAD HISTÓRICA', 'zoom' => 1.6],
            // Правая часть (после swap): берем элементы слева
            ['w' => $s3jStripW, 'img' => $termas1Path, 'srcX' => null, 'srcY' => null, 'caption' => null],
            ['w' => $s3jWideW, 'img' => $termas2Path, 'srcX' => null, 'srcY' => null, 'caption' => 'NATURALEZA Y ECOTURISMO'],
        ] as $idx => $panel) {
            $pw = $panel['w'];
            $px = $s3jX;
            $s3jX += $pw + $s3jGap;
        ...
        $mpdf->Cell(32, 8, 'Página 12', 0, 0, 'R');
        $s3hLineH = 0.5;
        $s3hLineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        ...
        $mpdf->Cell($s3hTextW, 13, 'CULTURA E', 0, 1, 'L');
        $mpdf->SetTextColor(255, 255, 255);
        $mpdf->SetFont('dejavusans', 'B', 34);
        $mpdf->SetXY($s3hTextX, $mpdf->y + 2);
        $mpdf->Cell($s3hTextW, 13, 'IDENTIDAD', 0, 1, 'L');
        $mpdf->SetFont('dejavusans', '', 14);
        $mpdf->SetXY($s3hTextX, $mpdf->y + 10);
        $mpdf->MultiCell($s3hParaW, 6.5, 'Santiago del Estero, Madre de Ciudades, preserva un patrimonio cultural vivo que articula tradición, música, gastronomía y expresiones artísticas como parte de su posicionamiento territorial.', 0, 'L');
        $mpdf->SetFont('dejavusans', '', 18);
        $mpdf->SetXY($s3hTextX, $mpdf->y + 45);
        $mpdf->MultiCell($s3hTextW, 8, "IDENTIDAD CULTURAL Y\nPATRIMONIO", 0, 'L');
        ...
        $mpdf->Cell(32, 8, 'Página 13', 0, 0, 'R');
        $s3iLineH = 0.5;
        $s3iLineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        ...
        for ($col = 0; $col < $s3iColCount; $col++) {
            ...
            if ($col === 0) {
                ...
                $mpdf->Cell($s3iColW, $s3iTitleRowH, 'GASTRONOMÍA', 0, 1, 'L');
                $mpdf->SetFont('dejavusans', '', $s3iDescFont);
                $mpdf->SetXY($s3iColX, $mpdf->y + $s3iTitleGap);
                $mpdf->MultiCell($s3iColW, $s3iDescLineH, "Sabores tradicionales e\nidentidad local.\nFiestas Populares\nCelebraciones y encuentros\nculturales.", 0, 'L');
            } elseif ($col === 1) {
                $mpdf->SetFont('dejavusans', 'B', $s3iTitleFont);
                $mpdf->SetXY($s3iColX, $s3iCurY);
                $mpdf->Cell($s3iColW, $s3iTitleRowH, 'ARTESANÍA', 0, 1, 'L');
                $mpdf->SetFont('dejavusans', '', $s3iDescFont);
                $mpdf->SetXY($s3iColX, $mpdf->y + $s3iTitleGap);
                $mpdf->MultiCell($s3iColW, $s3iDescLineH, "Saberes ancestrales y\nproducción regional.", 0, 'L');
            } elseif ($col === 2) {
                $mpdf->SetFont('dejavusans', 'B', $s3iTitleFont);
                $mpdf->SetXY($s3iColX, $s3iCurY);
                $mpdf->Cell($s3iColW, $s3iTitleRowH, 'PATRIMONIO HISTÓRICO', 0, 1, 'L');
                $mpdf->SetFont('dejavusans', '', $s3iDescFont);
                $mpdf->SetXY($s3iColX, $mpdf->y + $s3iTitleGap);
                $mpdf->MultiCell($s3iColW, $s3iDescLineH, "Espacios y memoria cultural\nprovincial.", 0, 'L');
            } else {
                $mpdf->SetFont('dejavusans', 'B', $s3iTitleFont);
                $mpdf->SetXY($s3iColX, $s3iCurY);
                ...
                $mpdf->SetFont('dejavusans', '', $s3iDescFont);
                $mpdf->SetXY($s3iColX, $mpdf->y + $s3iTitleGap);
                $mpdf->MultiCell($s3iColW, $s3iDescLineH, "Música, danza y tradición\npopular.", 0, 'L');
            }
        }
        ...
        $mpdf->Cell(32, 8, 'Página 14', 0, 0, 'R');
        $s4LineH = 0.5;
        $s4LineGap = 21;
        $mpdf->SetFillColor(255, 255, 255);
        ...
        foreach ($companies as $emp) {
            ...
            $s5Rows = [
                ...
                ['Contacto', $contactoSlidePorEmpresa[$cid] ?? '-'],
            ];
        ...
        foreach ([0, 1] as $p6idx) {
            ...
            if ($path && file_exists($path)) {
                $renderCroppedImage($path, $p6x, $p6ImgY, $p6w, $p6ImgH);
            } else {
                $mpdf->SetFillColor(60, 60, 60);
        ...
        foreach ($productoSlidesChunks as $idx => $chunk) {
            ...
            foreach ($chunk as $k => $prod) {
                ...
                $mpdf->SetFont('dejavusans', '', 11);
                $mpdf->MultiCell($p7textW, $p7DataLineH, 'Exportación anual: ' . (trim($prod['annual_export'] ?? '') ?: '-'), 0, $p7Align);
                $mpdf->SetX($p7textX);
                $mpdf->MultiCell($p7textW, $p7DataLineH, 'Certificaciones: ' . (trim($prod['certifications'] ?? '') ?: '-'), 0, $p7Align);
                $mpdf->SetX($p7textX);
                $mpdf->MultiCell($p7textW, $p7DataLineH, 'Código Arancelario (NCM/HS): ' . (trim($prod['tariff_code'] ?? '') ?: '-'), 0, $p7Align);
            }
            $mpdf->SetDrawColor(0, 0, 0);
            $prodPageNum++;
        }
